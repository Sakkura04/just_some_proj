...
class Stack():
    ...
    def get_from_stack(self, elem):
        current = self.tail
        stack_new = self.__class__()
        while current is not None:
            stack_new.add_to_tail(self.pop())
            current = self.tail
            if stack_new.tail.value == elem:
                print('The searched elem was on the position:', self.size + 1)
                self.recreate_stack(current, stack_new)
                return
        current.recreate_stack(stack_new)
        print('No such elements')

    def recreate_stack(self, current, stack_new):
        while current is not None:
            self.add_to_tail(stack_new.pop())
            current = stack_new.tail

    def __len__(self):
        return self.size

    def add_to_tail(self, value):
        node = Node_stack(value)
...
if __name__ == '__main__':
    ...
    value = 7
    if (linked_list.binarySearch(value) == None):
        print('Value', value, 'not present')
    else:
        print('Value', value, 'Present')
    ...
    Stack.add_to_tail(4)
    Stack.add_to_tail(5)
    print('The top is: ', Stack.top())
    print('Searching the elem 3: ', end='\n     ')
